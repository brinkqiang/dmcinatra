...
int main()
{
    ...
    server.init_ssl_context(true, [](auto, auto)
    {
        return "123456";
    ...
    person p{ 2 };
    server.set_http_handler<GET, POST>("/a", &person::foo, enable_cache{ false },
                                       log_t{});
    ...
    server.set_http_handler<GET, POST>("/test.html", [](request& req,
                                       response& res)
    {
        nlohmann::json json;
        json["number"] = 1024;
        ...
        res.set_status_and_content(status_type::ok,
                                   render::render_file("./www/test.html", json), cinatra::req_content_type::html);
    });

    server.set_http_handler<GET, POST>("/index.html", [](request& req,
                                       response& res)
    {
        res.add_header("Content-Type", "text/html; charset=utf-8");
        res.set_status_and_content(status_type::ok,
                                   render::render_file("./www/index.html"), cinatra::req_content_type::html);
    });
    ...
    server.set_http_handler<GET, POST>("/restype", [](request& req, response& res)
    {
        ...
        if (type == "html")
        {
            ...
            res.set_status_and_content(status_type::ok,
                                       "<a href='http://www.baidu.com'>hello world 百度</a>", res_type,
        ...
        else if (type == "json")
        {
            ...
            res.set_status_and_content(status_type::ok, json.dump(), res_type,
                                       content_encoding::none);
        }
        else if (type == "string")
        {
    ...
    server.set_http_handler<GET, POST>("/upload_octet_stream", [](request& req,
                                       response& res)
    {
